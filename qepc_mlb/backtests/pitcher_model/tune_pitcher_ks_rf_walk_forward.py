...
"""
QEPC-MLB pitcher strikeout Random Forest walk-forward tuner.
...
It deliberately excludes same-game lineup outcome columns such as:
  - lineup_batting_ks
  - lineup_batting_pa
...
SAFETY_VERSION = "rf_tune_pitcher_ks_noleak_v3_edge_leaf_grid"
...
def ensure_core(df: pd.DataFrame) -> pd.DataFrame:
    ...
    numeric_like = set(BASE_NUMERIC_CANDIDATES + [TARGET_COL])
...
def run_config_walk_forward(
    ...
    _, numeric_candidates, categorical_candidates, feature_meta = prepare_feature_lists(df, feature_stack, savant_feature_set)
