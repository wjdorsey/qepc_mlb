...
ENV_NUMERIC = [
    "env_temp", "env_temp_sq",
    "env_windspeed", "env_windspeed_sq",
    "env_wind_out_effect", "env_wind_in_effect", "env_wind_cross_effect",
    "env_temp_night_interaction",
]
ENV_CATEGORICAL = [
    "env_park_id", "env_daynight", "env_winddir", "env_usedh",
    "env_is_night", "env_is_day", "env_has_dh", "env_no_dh",
    "env_wind_out", "env_wind_in", "env_wind_cross", "env_wind_calm",
    "env_temp_missing", "env_windspeed_missing",
]



def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Walk-forward backtest for QEPC-MLB pitcher strikeouts with Savant, opponent, and game-environment context.")
    p.add_argument("--input", required=True, help="Path to pitcher game logs parquet/csv (baseline or enriched).")
    p.add_argument("--output_dir", default="artifacts/mlb/backtests/pitcher_strikeouts_walk_forward_env", help="Directory for outputs.")
    p.add_argument("--predictions_out", default="artifacts/mlb/backtests/pitcher_strikeouts_walk_forward_env/predictions.parquet")
...
def normalize(df: pd.DataFrame) -> pd.DataFrame:
    ...
    ] + [c for c in out.columns if c.startswith("savant_") or c.startswith("opp_team_") or c.startswith("env_")]
...
def choose_env_cols(df: pd.DataFrame) -> tuple[list[str], list[str]]:
    numeric = [c for c in ENV_NUMERIC if c in df.columns]
    categorical = [c for c in ENV_CATEGORICAL if c in df.columns]
    return numeric, categorical


def build_feature_lists(df: pd.DataFrame, use_enrichment: bool, savant_feature_set: str) -> tuple[list[str], list[str], list[str], list[str], list[str], list[str], list[str], list[str]]:
    ...
    env_num_cols, env_cat_cols = choose_env_cols(df) if use_enrichment else ([], [])
    categorical_candidates = [c for c in BASE_CATEGORICAL if c in df.columns] + env_cat_cols
    feature_cols = unique_keep_order([c for c in BASE_NUMERIC + savant_cols + opp_cols + env_num_cols + categorical_candidates if c in df.columns])
    ...
    return numeric_cols, categorical_cols, dropped_all_empty, dropped_non_modelable, savant_cols, opp_cols, env_num_cols, env_cat_cols
...
def run_walk_forward(df: pd.DataFrame, *, use_enrichment: bool, savant_feature_set: str, period: str, min_train_rows: int,
    ...
    candidate_numeric_cols, candidate_categorical_cols, dropped_all_empty_global, dropped_non_modelable_global, savant_candidates, opp_candidates, env_num_candidates, env_cat_candidates = build_feature_lists(
    ...
    summary = {
        ...
        "candidate_env_numeric_features": env_num_candidates,
        "candidate_env_categorical_features": env_cat_candidates,
...
def main():
    ...
    summary_input = {
        ...
        "env_context_cols": [c for c in df.columns if c.startswith("env_")],
        "env_row_rate": float(df["has_env_context"].fillna(False).mean()) if "has_env_context" in df.columns else None,
    }
    print("Input summary:")
