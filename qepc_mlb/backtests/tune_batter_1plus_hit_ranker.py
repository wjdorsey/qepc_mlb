...
# Pregame-safe prior batter performance splits vs RHP/LHP.
PLATOON_NUMERIC_FEATURES = [
    "batter_games_prior_vs_pitcher_hand",
    "batter_hit_1plus_rate_prior_vs_pitcher_hand",
    "batter_ba_prior_vs_pitcher_hand",
    "batter_k_rate_prior_vs_pitcher_hand",
    "batter_bb_rate_prior_vs_pitcher_hand",
    "batter_tb_per_ab_prior_vs_pitcher_hand",
]
...
def infer_features(df: pd.DataFrame, confirmed_lineups: bool, include_platoon_splits: bool = False) -> Tuple[List[str], List[str], Dict[str, List[str]]]:
    cols = set(df.columns)
    ...
    numeric = []
    for c in BASE_NUMERIC_FEATURES + rolling_features + MATCHUP_NUMERIC_FEATURES + (PLATOON_NUMERIC_FEATURES if include_platoon_splits else []):
    ...
    meta = {
        ...
        "platoon_splits": [
            c for c in valid_numeric
            if include_platoon_splits and c in set(PLATOON_NUMERIC_FEATURES)
        ],
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--include_platoon_splits", action="store_true", help="Include experimental batter vs pitcher-hand platoon split features.")
...
def main() -> None:
    ...
    numeric_features, categorical_features, feature_meta = infer_features(df, confirmed_lineups=args.confirmed_lineups, include_platoon_splits=args.include_platoon_splits)
